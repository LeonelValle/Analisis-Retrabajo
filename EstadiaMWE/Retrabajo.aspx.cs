...
namespace EstadiaMWE
{
    public partial class Retrabajo : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                if (!IsPostBack)
                {
                    ...
                    LlenarGrid(Conexion.ConsultaUnidadEstatus(Conexion.ConsultaEstatus("RWK")));
        ...
        protected void gvDefectos2_SelectedIndexChanged(object sender, EventArgs e)
        {
            EliminarDefecto();
        }

        protected void gvDefectos2_RowCreated(object sender, GridViewRowEventArgs e)
        {
            if (e.Row.Cells.Count > 3)
            {
                e.Row.Cells[1].Visible = false;
        ...
        protected void gvDefectos2_RowDataBound(object sender, GridViewRowEventArgs e)
        {
        ...
        protected void btnNuevo_Click(object sender, EventArgs e)
        {
            ...
            {
                pageControl.esNuevaUnidad = false;
                btnNuevo.Text = "Retrabajo rapido";
                modificarInfoAlta(false);
            }
            else
            {
                pageControl.esNuevaUnidad = true;
                btnNuevo.Text = "Retrabajando...";
        ...
        {
            if (pageControl.defectos_temporales.Count != 0 || refresh)
            {
                gvDefectos2.DataSource = pageControl.defectos_temporales;
                gvDefectos2.DataBind();
            }
        }
        protected void RefrescarGridDefectos()
        {
            pageControl.defectos_seleccionados.Clear();
            pageControl.defectos_temporales.Clear();
        ...
        public void RefrescarGrid()
        {
            if (esBusqueda())
            {
                LlenarGrid(Conexion.ConsultaUnidadFiltro(Conexion.ConsultaEstatus("RWK"), txtBuscarWO.Text, txtBuscarSN.Text));
            }
            else
            {
                LlenarGrid(Conexion.ConsultaUnidadEstatus(Conexion.ConsultaEstatus("RWK")));
        ...
        public void CambiarEstatusUnidad(string estatus)
        {
            if (CamposValidos())
            {
                ...
                foreach (Modelos.Defecto defecto in pageControl.defectos_temporales)
                {
                    pageControl.defectos_seleccionados.Add(defecto);
                }
        ...
        protected void modificarInfoAlta(bool modificable)
        {
            ...
            lblObligatorio5.Visible = modificable;
            lblObligatorio4.Visible = modificable;
        ...
        protected bool CamposValidos()
        {
            ...
            if (pageControl.esNuevaUnidad)
            {
                ...
                && ddlArea.SelectedValue != "-1" && gvDefectos2.Rows.Count > 0)
                {
                    return true;
                }
            }
        ...
        protected void AgregarDefecto()
        {
            ...
            if (txtReferencia.Text != "" && ddlDefecto.SelectedValue != "-1")
            {
                if (!pageControl.defectos_seleccionados.Any(x => x.Id_Defecto == Convert.ToInt32(ddlDefecto.SelectedValue)
                     && x.Referencia == txtReferencia.Text) && !pageControl.defectos_temporales.Any(x => x.Id_Defecto == Convert.ToInt32(ddlDefecto.SelectedValue)
                      && x.Referencia == txtReferencia.Text))
                {
                    pageControl.defectos_temporales.Add(new Modelos.Defecto
                    {
        ...
        {
            var itemToRemove = pageControl.defectos_temporales.Single(x => x.Id_Defecto == Convert.ToInt32(gvDefectos2.SelectedRow.Cells[1].Text)
                     && x.Referencia == gvDefectos2.SelectedRow.Cells[5].Text);

            pageControl.defectos_temporales.Remove(itemToRemove);
